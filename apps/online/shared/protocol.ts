...
export const chatTurnSchema = z.object({
  ...
  target: z.object({ mode: editModeSchema, pageIds: z.array(z.string()).default([]) }),
...
const editChangeSchema = z.object({
  kind: z.enum(['preserve_fact', 'rewrite_text', 'layout_change', 'style_change', 'image_replace', 'unsupported']),
...
export const editPlanSchema = z.object({
  ...
  estimatedCost: z.object({ imageUnits: z.number().nonnegative().max(1000), amount: z.number().nonnegative().max(1_000_000), currency: z.string().length(3) }),
