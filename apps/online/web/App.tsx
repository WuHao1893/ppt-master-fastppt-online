...
function ChatMessage({ message, busy, onRollbackOperation }: { message: ConversationMessage; busy: boolean; onRollbackOperation: (operationId: string) => Promise<void> }): JSX.Element {
...
function HistoryPanel({ page, compareId, onCompare, onRollback, onClose }: { page: Page; compareId: string; onCompare: (id: string) => void; onRollback: (id: string) => Promise<void>; onClose: () => void }): JSX.Element {
  return <div className="history-panel"><div className="history-heading"><div><span className="eyebrow">IMMUTABLE VERSIONS</span><strong>{page.versions.length} 个版本</strong></div><button className="icon-button" title="关闭历史" onClick={onClose}><X size={15} /></button></div><div className="history-list">{[...page.versions].reverse().map((version, index) => <div className={`history-row ${version.versionId === page.currentVersionId ? 'current' : ''}`} key={version.versionId}><div className="history-marker">{index === 0 ? <Check size={12} /> : <Clock3 size={12} />}</div><div className="history-info"><strong>{index === 0 ? '当前版本' : `版本 ${page.versions.length - index}`}</strong><span>{version.versionId} · {formatTime(version.createdAt)}</span><small>{version.message}</small></div><div className="history-actions"><button className="icon-button subtle" title="对比版本" onClick={() => onCompare(compareId === version.versionId ? '' : version.versionId)}><Copy size={14} /></button>{version.versionId !== page.currentVersionId && <button className="icon-button subtle" title="恢复此版本" onClick={() => void onRollback(version.versionId)}><RotateCcw size={14} /></button>}</div></div>)}</div>{compareId && <div className="compare-note"><Copy size={13} />预览区正在显示对比版本。再次点击可返回当前版本。</div>}</div>;
}
