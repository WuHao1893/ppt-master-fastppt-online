...
const complexPatterns =
  /复杂图表|结构化重绘|流程图连接线|自动重排连接线|复杂流程图|sankey|复杂图表/i;
const factTokenPattern =
  /(?<![\w])(?:\d+(?:\.\d+)?%?|\d{4}年?|第[一二三四五六七八九十]+[章节])/g;
const factReplacementPattern =
  /(?<![\w])((?:\d+(?:\.\d+)?%?|\d{4}年?|第[一二三四五六七八九十]+[章节]))\s*(?:改为|改成|替换为|调整为|变成|to)\s*((?:\d+(?:\.\d+)?%?|\d{4}年?|第[一二三四五六七八九十]+[章节]))/giu;
...
export function applyFactChanges(
  ...
  return anchors.map((anchor) => {
    const change = changes.find(
      (candidate) => candidate.oldValue === anchor.value,
    );
    return change ? { ...anchor, value: change.newValue } : { ...anchor };
  });
}

export function resolveSimilarPages(
  project: Project,
  message: string,
): { pageIds: string[]; reasons: Record<string, string> } {
  const terms = message
    .toLowerCase()
    .split(/\s+/)
    .filter((term) => term.length > 1);
  const scored = project.pages
    .map((page) => {
      const haystack =
        `${page.title} ${page.body} ${page.layout} ${page.contract.components.join(" ")}`.toLowerCase();
      const hits = terms.filter((term) => haystack.includes(term)).length;
      const structural =
        /标题|title/.test(message) && page.title.length > 18 ? 2 : 0;
      const score = hits + structural;
      const reason = structural
        ? "标题长度符合请求条件"
        : hits
          ? `合同/标题中命中 ${hits} 个相关词`
          : `布局标签为 ${page.layout}，与请求目标相近`;
  ...
  const fallback = selected.length
    ? selected
    : scored.filter((item) => item.page.pageType === "content").slice(0, 6);
  return {
    pageIds: fallback.map((item) => item.page.pageId),
    reasons: Object.fromEntries(
      fallback.map((item) => [item.page.pageId, item.reason]),
    ),
  };
}
...
function addChange(
  ...
  change: EditPlan["changes"][number],
): void {
  changes.push(change);
}

export function previewTextChange(
  page: Page,
  message: string,
): { title: string; body: string } {
  let title = page.title;
  let body = page.body;
  const explicitTitle = message
    .match(
      /(?:标题|title)(?:改成|改为|设为|[:：])\s*[“"']?([^。；;\n”"']{2,60})/i,
    )?.[1]
    ?.trim();
  if (explicitTitle) title = explicitTitle;
    ...
    /标题|title|一句话|改短|缩短|headline/i.test(message) &&
    title.length > 18
  )
    title = `${title.slice(0, 17)}…`;
  const explicitBody = message
    .match(
      /(?:正文|内容|body)(?:改成|改为|设为|[:：])\s*[“"']?([^”"']{4,500})/i,
    )?.[1]
    ?.trim();
...
function calculateFactImpact(
  ...
  for (const page of pages) {
    ...
    for (const fact of page.factAnchors) {
      if (!proposed.includes(fact.value))
        removed.add(`${fact.factId}:${fact.value}`);
    }
    const proposedFacts =
      proposed.match(
        /(?<![\w])(?:\d+(?:\.\d+)?%?|\d{4}年?|第[一二三四五六七八九十]+[章节])/g,
    ...
    for (const value of new Set(proposedFacts)) {
      if (!page.factAnchors.some((fact) => fact.value === value))
        added.add(value);
    }
    for (const change of requestedChanges) {
      const anchor = page.factAnchors.find(
        (fact) => fact.value === change.oldValue,
      );
...
export function buildEditPlan(
  ...
  const factIds = pages.flatMap((page) =>
    page.factAnchors.map((fact) => fact.factId),
  );
  ...
  const unsupported = complexPatterns.test(message)
    ? ["复杂图表/流程图结构化重绘不属于 MVP，原结构将被保留。"]
    : [];
  ...
  if (
    /两列|双栏|三列|三阶段|布局|右侧|左侧|column|layout|timeline|路线图/.test(
      normalized,
    )
  ) {
    const value = /三阶段|timeline|路线图/.test(normalized)
      ? "timeline"
      : /三列/.test(normalized)
        ? "three-column"
        : "two-column";
  ...
  if (changes.every((change) => change.kind === "preserve_fact")) {
    addChange(changes, {
      kind: "rewrite_text",
      target: "body",
      constraint: "保持事实锚点，优化表达密度",
      value: message,
    });
  }
  const imageUnits = changes.some((change) => change.kind === "image_replace")
    ? pages.length
    : 0;
  const factImpact = calculateFactImpact(pages, message);
  const confirmationReasons: string[] = [];
  if (mode !== "single")
    confirmationReasons.push(`${mode} 范围必须确认影响页面。`);
  if (unsupported.length)
    confirmationReasons.push("计划含当前不能结构化执行的内容。");
  if (factImpact.removed.length || factImpact.changed.length)
    confirmationReasons.push("计划会改变已登记事实。");
  if (/删除事实|删除数字|替换主题|改变页数/.test(message))
    confirmationReasons.push("请求涉及事实、主题或页面数量变化。");
  const visualChange =
    context.workflowMode === "pptx_beautify" ||
    changes.some((change) =>
  ...
  if (visualChange)
    confirmationReasons.push("视觉修改必须先审阅视觉候选，再确认可编辑重建。");
  const requiresConfirmation = confirmationReasons.length > 0;
  if (factImpact.removed.length > 0) {
    unsupported.push(
      `检测到将移除锁定事实：${factImpact.removed.join("、")}。确认前不会执行。`,
    );
  }
  return {
    workflowMode: context.workflowMode || "page_entry",
    targetScope: mode,
    intent:
      context.intent ||
      (imageUnits ? "layout_and_visual_revision" : "structured_page_revision"),
    affectedPageIds: pages.map((page) => page.pageId),
    pageDelta: { add: [], remove: [], split: [], merge: [] },
    changes,
    factImpact,
    sourceDocumentIds: context.sourceDocumentIds || [],
    conflictIds: context.conflictIds || [],
    unsupported,
    requiresConfirmation,
    confirmationReasons,
    estimatedCost: { imageUnits, amount: imageUnits * 0.08, currency: "USD" },
    summary: unsupported.length
      ? "已识别请求，但复杂结构保留原样并等待确认。"
      : `${pages.length} 个页面将按合同约束执行：${message}`,
  };
}
...
export function validateEditPlan(
  ...
  if (plan.targetScope !== mode)
    errors.push("计划 targetScope 与请求范围不一致。");
  if (
    actual.size !== expected.size ||
    [...expected].some((pageId) => !actual.has(pageId))
  )
    errors.push("计划影响页必须与已解析的目标 page_id 完全一致。");
  if (plan.affectedPageIds.length !== actual.size)
    errors.push("计划影响页不能包含重复 page_id。");
  if (mode !== "single" && !plan.requiresConfirmation)
    errors.push("多页和全局修改必须经过确认门。");
  if (
    (plan.factImpact.removed.length ||
      plan.factImpact.changed.length ||
      plan.unsupported.length) &&
    !plan.requiresConfirmation
  )
    errors.push("事实变化或不支持项必须经过确认门。");
  const knownFacts = new Set(
    pages.flatMap((page) =>
      page.factAnchors.map((fact) => `${fact.factId}:${fact.value}`),
    ),
  );
  for (const item of plan.factImpact.removed) {
    if (item.includes(":") && !knownFacts.has(item))
      errors.push(`计划删除了不存在的事实锚点：${item}。`);
  }
  for (const item of plan.factImpact.changed) {
    const match = item.match(/^([^:]+):(.+)->(.+)$/);
    if (!match || !knownFacts.has(`${match[1]}:${match[2]}`) || !match[3])
      errors.push(`事实替换格式或来源无效：${item}。`);
  }
  const imageChange = plan.changes.some(
    (change) => change.kind === "image_replace",
  );
  const expectedUnits = imageChange ? pages.length : 0;
  if (plan.estimatedCost.imageUnits !== expectedUnits)
    errors.push(`图像成本预估不一致，应为 ${expectedUnits} image unit。`);
  if (plan.estimatedCost.amount < 0 || plan.estimatedCost.amount > 1_000_000)
    errors.push("成本预估超出允许范围。");
  return errors;
}
...
export function composePrompt(
  ...
  context: {
    ...
    conflictResolutions?: Array<Record<string, unknown>>;
    model?: string;
    image?: { width: number; height: number; quality: string };
  } = {},
  ...
  const prompt = [
    ...
    `Current revision: ${project.currentDeckRevisionId}; current version: ${page.currentVersionId}; parent version: ${currentVersion?.parentVersionId || "none"}.`,
    `Model: ${context.model || "deterministic-local-planner"}; image parameters: ${JSON.stringify(context.image || { width: 1600, height: 900, quality: "high" })}; cost budget: ${JSON.stringify(plan.estimatedCost)}.`,
    `Registered sources: ${JSON.stringify(context.sources || [])}.`,
    `Conflict decisions: ${JSON.stringify(context.conflictResolutions || [])}.`,
